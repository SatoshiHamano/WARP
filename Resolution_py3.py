...
# Usage:
#   python Resolution_py3.py <speclist> <linelist> <output dat> (-s 100) (-m WIDE)

def gaussianfunc(p,x,c):
    y = p[0] * np.exp(-(x-p[1]-c)**2 / (2 * p[2] **2) ) + p[3]
    return y
...
def resolution_measure(fitsf, linef, output, mode, slit):
    ...
    pp = PdfPages(output.rstrip(output.split(".")[-1]) + "pdf")
    fig = plt.figure()
    ...
    for l in range(len(rlines)):
        ...
        for i in range(len(wave[l])):
            ...
            if minlpix > 1 and maxlpix < aplength:
                p0 = [1.e+3, 0.01, base_res / wave[l][i], np.median(apdata[minlpix:maxlpix])]
                param_output = scipy.optimize.leastsq(residue, p0, args=(
                apdata[minlpix:maxlpix], alamarray[minlpix:maxlpix], wave[l][i]), full_output=True)

                if wave[l][i] / math.fabs(param_output[0][2] * 2.35) > base_res / 3. and wave[l][i] / math.fabs(
                        param_output[0][2] * 2.35) < base_res * 3.:
                    wave_ch[l].append(wave[l][i])
                    peak[l].append(param_output[0][0])
                    centerlams[l].append(param_output[0][1])
                    gwidth[l].append(param_output[0][2] * 2.35)

                    plt.scatter(alamarray[minlpix:maxlpix], apdata[minlpix:maxlpix])
                    plt.plot(alamarray[minlpix:maxlpix],
                             gaussianfunc(param_output[0], alamarray[minlpix:maxlpix], wave[l][i]))
    ...
    plt.grid()
    plt.savefig(output.rstrip(output.split(".")[-1]) + "png", format="png")
...
if __name__ == '__main__':
    ...
    parser.add_argument("speclist", type=str, help="Ascii file listing spectra fits files.")
    parser.add_argument("linelist", type=str, help="Ascii file listing emission line info.")
    parser.add_argument("output", type=str, help="Output ascii file")
    parser.add_argument("-m", "--mode", help="Observational mode", choices=["WIDE", "HIRES-Y", "HIRES-J"],
                        default="WIDE")
    parser.add_argument("-s", "--slit", type=int, help="Slit width in micron", default=100)
    ...
    resolution_measure(speclist, linelist, output, mode, slit)
